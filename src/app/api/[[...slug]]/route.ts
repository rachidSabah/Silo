...
async function handleRequest(req: NextRequest) {
  ...
  try {
    switch (true) {
      ...
      case path === 'ai/bulk-generate' && m === 'POST': { const d = await body(req); const result = await generateSiloAwareArticle(d.pages?.[0]?.title || '', d.pages?.[0]?.type || 'pillar', d.pages?.[0]?.keywords || [], { siloName: d.siloName, pillarPage: null, siblingPages: [], internalLinks: [], brandVoice: d.brandVoice, niche: d.niche }, 2000, req); return json({ article: result }); }
      // Import/Export
