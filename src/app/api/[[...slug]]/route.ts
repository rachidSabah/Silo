...
import { runGeoGridScan, getSERPCredentials } from '@/lib/serp-api';
import { getGBPCredentials, generateAuthURL as generateGBPAuthURL, exchangeCodeForTokens, refreshAccessToken, fetchGBPLocations as fetchGBPApiLocations, fetchGBPReviews as fetchGBPApiReviews, publishGBPPost as publishGBPApiPost } from '@/lib/gbp-oauth';
...
async function handleRequest(req: NextRequest) {
  ...
  try {
    switch (true) {
      ...
      // ──── Local SEO: GBP OAuth ────
      case path === 'gbp-auth' && m === 'GET': { const u = await getUserFromRequest(req); if (!u) return NextResponse.json({ error: 'Not authenticated' }, { status: 401 }); const cred = getGBPCredentials(); if (!cred.clientId) return NextResponse.json({ error: 'GBP OAuth not configured. Set GBP_CLIENT_ID and GBP_CLIENT_SECRET secrets.' }, { status: 500 }); const origin = new URL(req.url).origin; return NextResponse.redirect(generateGBPAuthURL(origin, u.userId)); }
      case path === 'gbp-auth/callback' && m === 'GET': { const code = url.searchParams.get('code'); const err = url.searchParams.get('error'); const state = url.searchParams.get('state') || ''; const o = new URL(req.url).origin; if (err || !code) return NextResponse.redirect(`${o}/?gbp_error=${err || 'no_code'}`); const cred = getGBPCredentials(); if (!cred.clientId || !cred.clientSecret) return NextResponse.redirect(`${o}/?gbp_error=config_missing`); try { const tokens = await exchangeCodeForTokens(code, o); const f = new URL('/', o); f.hash = `gbp_access_token=${encodeURIComponent(tokens.access_token)}&gbp_refresh_token=${encodeURIComponent(tokens.refresh_token || '')}&gbp_expires_in=${tokens.expires_in || 3600}&gbp_user_id=${state}`; return NextResponse.redirect(f.toString()); } catch (e: unknown) { const msg = e instanceof Error ? e.message : 'token_exchange_failed'; return NextResponse.redirect(`${o}/?gbp_error=${encodeURIComponent(msg)}`); } }
      case path === 'gbp-auth/refresh' && m === 'POST': { const d = await body(req); if (!d.refresh_token) return NextResponse.json({ error: 'refresh_token required' }, { status: 400 }); try { const tokens = await refreshAccessToken(d.refresh_token); return json(tokens); } catch (e: unknown) { return NextResponse.json({ error: e instanceof Error ? e.message : 'Refresh failed' }, { status: 400 }); } }
      case path === 'gbp/fetch-locations' && m === 'POST': { const u = await getUserFromRequest(req); if (!u) return NextResponse.json({ error: 'Not authenticated' }, { status: 401 }); const d = await body(req); if (!d.access_token) return NextResponse.json({ error: 'access_token required' }, { status: 400 }); try { const apiLocations = await fetchGBPApiLocations(d.access_token); const mapped = apiLocations.map(loc => ({ location_name: loc.title || '', address: loc.storefrontAddress?.addressLines?.join(', ') || null, phone: loc.phoneNumbers?.primaryPhone || null, lat: loc.latlng?.latitude || null, lng: loc.latlng?.longitude || null, category: loc.categories?.primaryCategory?.displayName || null, website_url: loc.websiteUri || null, location_id: loc.name || null })); return json({ locations: mapped }); } catch (e: unknown) { return NextResponse.json({ error: e instanceof Error ? e.message : 'Failed to fetch locations' }, { status: 500 }); } }
      ...
      case path === 'geogrid/scan' && m === 'POST': { const u = await getUserFromRequest(req); if (!u) return NextResponse.json({ error: 'Not authenticated' }, { status: 401 }); if (!checkRateLimit(`geogrid_${u.userId}`, 5, 300_000)) return NextResponse.json({ error: 'Rate limit exceeded. Max 5 scans per 5 minutes.' }, { status: 429 }); return await handleGeoGridScan(req); }
      ...
      case path === 'gbp/posts' && m === 'POST': { const u = await getUserFromRequest(req); if (!u) return NextResponse.json({ error: 'Not authenticated' }, { status: 401 }); const d = await body(req); await createGBPPost({ id: d.id || crypto.randomUUID(), gbp_location_id: d.gbp_location_id, project_id: d.project_id, post_type: d.post_type || 'update', title: d.title || null, content: d.content || '', media_url: d.media_url || null, call_to_action: d.call_to_action || null, link_url: d.link_url || null, start_date: d.start_date || null, end_date: d.end_date || null, status: d.status || 'draft', google_post_id: d.google_post_id || null, scheduled_at: d.scheduled_at || null, published_at: d.published_at || null }); if (d.publish && d.gbp_location_id && d.project_id) { try { const locations = await getGBPLocations(d.project_id); const loc = (locations as any[]).find((l: any) => l.id === d.gbp_location_id); if (loc?.access_token && loc?.location_id) { await publishGBPApiPost(loc.access_token, loc.location_id, { summary: d.content, topicType: d.post_type === 'offer' ? 'OFFER' : d.post_type === 'event' ? 'EVENT' : 'STANDARD' }); } } catch (e) { console.error('[GBP] Publish to Google failed:', e); } } return json({ ok: true }); }
...
async function handleGeoGridScan(req: NextRequest) {
  ...
  // Get SERP provider info
  const serpCreds = getSERPCredentials();
  const providerLabel = serpCreds.provider === 'mock' ? 'Mock (no API key)' : serpCreds.provider;

  // Cost warning: large scans are expensive
  if (totalNodes > 25) {
    console.log(`[GeoGrid] Large scan: ${size}x${size} = ${totalNodes} nodes via ${providerLabel}`);
  }
  ...
  try {
    // Use real SERP API (auto-selects DataForSEO, Google, or Mock based on env config)
    const scanResults = await runGeoGridScan({
      keyword, center_lat, center_lng,
      grid_size: size, radius: rad,
      business_name: business_name || '',
    });

    // Convert results to DB format
    const nodes = scanResults.map(r => ({
      id: crypto.randomUUID(), scan_id: scanId, lat: r.lat, lng: r.lng,
      row_idx: r.row_idx, col_idx: r.col_idx, rank: r.rank,
      business_name: r.rank && r.rank <= 3 ? (business_name || 'Your Business') : null,
    ...
    return json({ scan, nodes, provider: providerLabel });
  } catch (err: unknown) {
    const msg = err instanceof Error ? err.message : 'Scan failed';
    console.error(`[GeoGrid] Scan ${scanId} failed:`, msg);
    await updateGeoGridScan(scanId, { status: 'failed', completed_at: new Date().toISOString() });
    return NextResponse.json({ error: `GeoGrid scan failed: ${msg}`, scan_id: scanId }, { status: 500 });
  }
}
